...
assert.equal((statisticsHtml.match(/data-collapsible/g) || []).length, 6);
assert.equal((statisticsHtml.match(/class="section-toggle"/g) || []).length, 6);
assert.equal((statisticsHtml.match(/section-updated/g) || []).length, 6);
assert.match(statisticsHtml, /aria-labelledby="traffic-title"[\s\S]*aria-labelledby="clicks-title"[\s\S]*aria-labelledby="audience-title"/);
assert.match(statisticsHtml, /id="click-offers"[\s\S]*id="click-booking"[\s\S]*id="click-ryanair"[\s\S]*id="click-wizz"/);
assert.match(statisticsHtml, /id="audience-freshness"/);
...
assert.match(statisticsJs, /runs-freshness/);
assert.match(engagementJs, /data-stat-click/);
assert.match(engagementJs, /keepalive: true/);
