...
assert.equal(collapseToggle.getAttribute("aria-label"), "Zobraziť vyhľadávanie");
...
assert.match(html, /class="table-note table-note-bottom"[^>]*>Cena tam \+ späť/);
assert.match(html, /id="results-content"[\s\S]+class="table-note table-note-bottom"/);
...
assert.match(html, /class="filter-panel collapsible-section"[\s\S]+aria-expanded="true"[^>]+aria-controls="filters-content"/);
assert.doesNotMatch(html, /id="filters-content"[^>]*hidden/);
...
assert.match(html, /data-i18n="contact\.eyebrow">Kontakt<\/span>[\s\S]+data-i18n="contact\.title">Podpora a Kontakt<\/h2>/);
